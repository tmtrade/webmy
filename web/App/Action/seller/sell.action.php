...
class SellAction extends AppAction {
    ...
    public function getTminfo() {
        ...
        if (empty($number))
            $this->returnAjax(array('code' => 1, 'msg' => '商标号不能为空'));
        //判断用户是否已出售过
        ...
        if (empty($info))
            $this->returnAjax(array('code' => 3, 'msg' => '找不到对应商标，请查证重新输入'));
        //不能出售的商标
    ...
    public function getPerson() {
        ...
        if (!$person)
            return array('code' => 1, 'msg' => '申请人不能为空');
    ...
    public function changeContact() {
        ...
        if (!$name)
            $this->returnAjax(array('code' => 1, 'msg' => '联系人为空'));
        //保存用户的姓名
